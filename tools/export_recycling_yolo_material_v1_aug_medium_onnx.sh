#!/usr/bin/env bash
# Export the medium-augmentation recycling_yolo_material_v1 checkpoint
# (see tools/train_recycling_yolo_material_v1_aug_medium.sh) to ONNX,
...
TRAINED_MODEL="${TRAINED_MODEL:-runs/recycling_yolo/yolo11n_material_v1_aug_medium_640/weights/best.pt}"
OUTPUT="${OUTPUT:-models/yolo11n_recycling_material_v1_aug_medium_640.onnx}"
IMGSZ="${IMGSZ:-640}"

if [ ! -f "$TRAINED_MODEL" ]; then
  echo "ERROR: trained model not found at ${TRAINED_MODEL}" >&2
  echo "       Run tools/train_recycling_yolo_material_v1_aug_medium.sh" >&2
...
echo "== Exporting recycling YOLO (material_v1, aug=medium) to ONNX =="
